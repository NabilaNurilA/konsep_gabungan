...
if ($search) {
    $sql .= " WHERE LOWER(full_name) LIKE ? OR LOWER(email) LIKE ? OR LOWER(phone) LIKE ? OR LOWER(position) LIKE ? OR LOWER(gender) LIKE ?";
    $searchTerm = "%" . $search . "%";
...
if ($search) {
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssiii", $searchTerm, $searchTerm, $start_from, $limit);
} else {
    $stmt = $conn->prepare($sql);
...
<body>
    <div class="container">
        <h2>Pendaftar Anggota Klub Sepak Bola</h2>
